import os.path
from file_handler import write_to_csv
from cross_validation import calc_recogn_ratio, cross_validation
...
def test_winlen(file_list, csv_file_name):
    if not os.path.isfile(csv_file_name):
        ...
        win_lens = [0.01, 0.025, 0.05, 0.07, 0.1, 0.2, 0.25, 0.3, 0.035, 0.4, 0.45, 0.5, 0.55, 0.6]
        output = {}
        for winLen in win_lens:
            params_dict = get_mfcc_matrices_for_each_speaker(file_list, winlen=winLen, nfft=2048)
...
def test_numcep(file_list, csv_file_name):
    if not os.path.isfile(csv_file_name):
        ...
        for cep_num in cep_nums:
            ...
            params_dict = get_mfcc_matrices_for_each_speaker(file_list, numcep=cep_num,)
            confusion_matrix = cross_validation(params_dict)
            recogn_ratio = calc_recogn_ratio(confusion_matrix)
...
def test_nfilt(file_list, csv_file_name):
    if not os.path.isfile(csv_file_name):
        ...
        for filter_num in filter_nums:
            ...
            params_dict = get_mfcc_matrices_for_each_speaker(file_list, nfilt=filter_num)
            confusion_matrix = cross_validation(params_dict)
            recogn_ratio = calc_recogn_ratio(confusion_matrix)
...
def test_nfft(file_list, csv_file_name):
    if not os.path.isfile(csv_file_name):
        ...
        for nfft_num in nfft_nums:
            ...
            params_dict = get_mfcc_matrices_for_each_speaker(file_list, nfft=nfft_num)
            confusion_matrix = cross_validation(params_dict)
            recogn_ratio = calc_recogn_ratio(confusion_matrix)
...
def test_ncomponents(file_list, csv_file_name):
    if not os.path.isfile(csv_file_name):
        ...
        for ncomponents_num in ncomponents_nums:
            ...
            params_dict = get_mfcc_matrices_for_each_speaker(file_list)
...
def test_niters(file_list, csv_file_name):
    if not os.path.isfile(csv_file_name):
        ...
        for n_iter_num in n_iter_nums:
            ...
            params_dict = get_mfcc_matrices_for_each_speaker(file_list)
            confusion_matrix = cross_validation(params_dict, n_iter=n_iter_num)
            recogn_ratio = calc_recogn_ratio(confusion_matrix)
